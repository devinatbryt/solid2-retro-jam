...
export interface SessionData {
  userId?: string;
  /** Stable per-browser id for this jam's identity (see src/server/identity.ts). */
  jamId?: string;
  /** Display name auto-assigned on first visit. No login, no accounts. */
  jamName?: string;
  /** Hue (0-359) used to colour this person's contributions. */
  jamHue?: number;
}
